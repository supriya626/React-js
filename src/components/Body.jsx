    ...
    return localRestaurants.length === 0 ? <Shimmer /> : (
        <div className="mx-20">
            <div className="flex justify-between mx-20 my-8">
                <a className="text-3xl m-5 font-bold hover:underline hover:text-orange-300" href="#"
                ...
                <div className="flex justify-between items-center ">
                    <input className="text-base w-80 h-10 border-2 border-solid border-black rounded-md pl-2 focus:border-orange-300" type="text"
                        placeholder="enter the restaurant name..."
                        value={searchName}
                        onChange={e => setsearchName(e.target.value)} />
                    <button className="w-24 h-10 bg-orange-300 ml-2 cursor-pointer rounded-md hover:bg-orange-200"
                        onClick={() => setTempRestaurants(localRestaurants.filter(
